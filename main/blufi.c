                                ...
                                int32_t event_id, void* event_data)
{
    ...
    case IP_EVENT_STA_GOT_IP: {
        ...
        if (ble_is_connected == true) {
            ...
            BLUFI_INFO("BLUFI BLE is not connected yet, get wifi ip\n");
        }
        break;
    }
    default:
...
static void initialise_wifi(void)
{
    ...
    /*创建Wi-Fi驱动程序任务并初始化Wi-Fi驱动程序*/
    wifi_init_config_t cfg = WIFI_INIT_CONFIG_DEFAULT();
    ESP_ERROR_CHECK( esp_wifi_init(&cfg) );
    ESP_ERROR_CHECK( esp_wifi_set_mode(WIFI_MODE_STA) );
    ESP_ERROR_CHECK( esp_wifi_start() );/*进入esp_event_loop()创建的事件里面*/
}
/*回调函数和安全函数*/
static esp_blufi_callbacks_t example_callbacks = {
    .event_cb = example_event_callback,                              //BLUFI事件回调函数
    .negotiate_data_handler = blufi_dh_negotiate_data_handler,      //  DH算法秘钥协商函数
...
static void example_event_callback(esp_blufi_cb_event_t event, esp_blufi_cb_param_t *param)
{
    ...
    case ESP_BLUFI_EVENT_GET_WIFI_STATUS: {
        ...
        if (gl_sta_connected) {  
            ...
            esp_blufi_send_wifi_conn_report(mode, ESP_BLUFI_STA_CONN_FAIL, 0, NULL);
            BLUFI_INFO("Wifi disconnect from AP\n");
        }
    ...
    case ESP_BLUFI_EVENT_RECV_CUSTOM_DATA:
        ...
        esp_blufi_send_custom_data(param->custom_data.data, param->custom_data.data_len);//向手机端发送数据
        break;
...
void blufi_init(void)
{
    ...
    BLUFI_INFO("please connect to wifi");
    /* 等待WIFI连接信号量，死等*/
